...
template <class... Ts, class... T1s, class... T2s, class... T3s, class... T4s, class... T5s, class... T6s, class... T7s,
          class... T8s, class... T9s, class... T10s, class... T11s, class... T12s, class... T13s, class... T14s,
          class... T15s, class... T16s, class... Us>
struct join<type_list<Ts...>, type_list<T1s...>, type_list<T2s...>, type_list<T3s...>, type_list<T4s...>,
    ...
    : join<type_list<Ts..., T1s..., T2s..., T3s..., T4s..., T5s..., T6s..., T7s..., T8s..., T9s..., T10s..., T11s...,
                     T12s..., T13s..., T14s..., T15s..., T16s...>,
           Us...> {};
...
struct missing_ctor_parameter {
	...
	template <class U,
	          __BOOST_SML_REQUIRES(!aux::is_base_of<pool_type_base, U>::value && aux::is_constructible<U>::value)>
	...
	{
		static_assert(
		    missing_ctor_parameter<TMissing>::value,
...
const char *get_type_name()
{
	...
	return detail::get_type_name<T, 50>(__PRETTY_FUNCTION__,
...
#elif defined(__GNUC__)
	return detail::get_type_name<T, 69>(__PRETTY_FUNCTION__,
	                                    make_index_sequence<sizeof(__PRETTY_FUNCTION__) - 69 - 2>{});
#elif defined(__ICCARM__)
	return detail::get_type_name<T, 72>(__PRETTY_FUNCTION__,
...
struct get_sub_internal_events_impl {
	using type =
	    aux::conditional_t<aux::is_base_of<internal_event, TEvent>::value, aux::type_list<TEvent>, aux::type_list<>>;
};
...
using get_all_events = aux::join_t<
    typename get_all_events_impl<typename Ts::src_state, typename Ts::dst_state, typename Ts::event>::type...>;
template <class... Ts>
...
using get_initial_states = aux::join_t<
    typename aux::conditional<Ts::initial, aux::type_list<typename Ts::src_state>, aux::type_list<>>::type...>;
template <class... Ts>
...
struct sub_sm<sm_impl<sm_policy<T, aux::identity<U>>>> {
	...
	{
		return static_cast<const aux::pool_type<sm_impl<sm_policy<T, aux::identity<U>, TPolicies...>>> &>(*object)
		    .value;
	}
};
...
struct get_state_mapping {
	using type = aux::conditional_t<
	    aux::is_same<decltype(get_state_mapping_impl<T, TUnexpected>((TMappings *)0)), transitions<TUnexpected>>::value,
...
struct fold_expr {
	template <class TMappings, int... Ns, class sm_impl, class State, class TEvent, class TDeps, class TSubs,
	          class... TStates>
	constexpr static bool dispatch_impl(sm_impl &self, State &current_state, aux::index_sequence<Ns...>,
	...
	template <int N, class TMappings, class sm_impl, class State, class TEvent, class TDeps, class TSubs,
	          class... TStates>
	constexpr static bool dispatch(sm_impl &self, State &current_state, const TEvent &event, TDeps &deps, TSubs &subs,
	                               const aux::type_list<TStates...> &states)
	{
		return dispatch_impl<TMappings>(self, current_state, aux::make_index_sequence<sizeof...(TStates)>{}, event,
		                                deps, subs, states);
	}
};
#endif
...
template <class SM, class TLogger, class TDeps, class TAction, class TEvent>
constexpr void log_action(const aux::type<TLogger> &, TDeps &deps, const aux::zero_wrapper<TAction> &action,
                          const TEvent &event)
{
...
template <class SM, class TLogger, class TDeps, class TGuard, class TEvent>
constexpr void log_guard(const aux::type<TLogger> &, TDeps &deps, const aux::zero_wrapper<TGuard> &guard,
...
struct should_not_subclass_statemachine_class
    : integral_constant<bool,
                        is_empty<typename Tsm::sm>::value || should_not_instantiate_statemachine_class<Tsm>::value> {};
} // namespace aux
...
struct sm_policy {
	...
	using defer_queue_policy =
	    decltype(get_policy<no_policy, policies::defer_queue_policy__>((aux::inherit<TPolicies...> *)0));
...
template <class, class T>
struct callable
    : decltype(test_callable<aux::inherit<aux::conditional_t<__is_class(T), T, aux::none_type>, callable_fallback>>(
          0)) {};
...
template <class Tsm>
struct sm_impl
    : aux::conditional_t<aux::should_not_subclass_statemachine_class<Tsm>::value, aux::none_type, typename Tsm::sm> {
...
#else
		return process_event_except_impl<get_event_mapping_t<get_generic_t<TEvent>, mappings>>(event, deps, subs,
		                                                                                       has_exceptions{});
#endif
	}
	template <class TEvent, class TDeps, class TSubs,
	...
	constexpr bool process_internal_events(const TEvent &event, TDeps &deps, TSubs &subs)
	{
		...
		return process_event_impl<get_event_mapping_t<get_mapped_t<TEvent>, mappings>>(
		    event, deps, subs, states_t{}, aux::make_index_sequence<regions>{});
#else
		return process_event_except_impl<get_event_mapping_t<get_mapped_t<TEvent>, mappings>>(event, deps, subs,
		                                                                                      has_exceptions{});
#endif
	}
	template <class TEvent, class TDeps, class TSubs, class... Ts,
	...
	constexpr bool process_event_impl(const TEvent &event, TDeps &deps, TSubs &subs,
	                                  const aux::type_list<TStates...> &states, aux::index_sequence<0>)
	{
	...
	constexpr bool process_event_impl(const TEvent &event, TDeps &deps, TSubs &subs,
	                                  const aux::type_list<TStates...> &states, aux::index_sequence<Ns...>)
	{
		auto handled = false;
#if defined(__cpp_fold_expressions)
		((handled |= dispatch_t::template dispatch<0, TMappings>(*this, current_state_[Ns], event, deps, subs, states)),
		 ...);
#else
		(void)aux::swallow{0, (handled |= dispatch_t::template dispatch<0, TMappings>(*this, current_state_[Ns], event,
		                                                                              deps, subs, states),
		                       0)...};
#endif
		return handled;
	...
	template <class TMappings, class TEvent, class TDeps, class TSubs, class... TStates>
	constexpr bool process_event_impl(const TEvent &event, TDeps &deps, TSubs &subs,
	                                  const aux::type_list<TStates...> &states, state_t &current_state)
	{
	...
	template <class TMappings, class TEvent, class TDeps, class TSubs>
	constexpr bool process_event_except_impl(const TEvent &event, TDeps &deps, TSubs &subs, state_t &current_state,
	                                         aux::false_type)
	{
	...
	template <class TDeps, class TSubs, class TDeferQueue, class... TEvents>
	bool process_queued_events(TDeps &deps, TSubs &subs, bool &queued_handled, const aux::type<TDeferQueue> &,
	                           const aux::type_list<TEvents...> &)
	{
		using dispatch_table_t = bool (sm_impl::*)(TDeps &, TSubs &, const void *);
		const static dispatch_table_t dispatch_table[__BOOST_SML_ZERO_SIZE_ARRAY_CREATE(sizeof...(TEvents))] = {
	...
	constexpr bool is_terminated_impl(aux::index_sequence<Ns...>) const
	{
		...
		auto result = true;
		(void)aux::swallow{
		    0, (result &= current_state_[Ns] == aux::get_id<state_t, terminate_state>((states_ids_t *)0))...};
		return result;
#endif
	}
...
class sm {
  ...
  private:
	using sm_all_t = aux::apply_t<
	    get_non_empty_t,
	    aux::join_t<
	        aux::conditional_t<aux::is_same<no_policy, typename Tsm::dont_instantiate_statemachine_class_policy>::value,
...
#else
		(void)aux::swallow{
		    0, (sm.current_state_[region++] = aux::get_id<state_t, typename TStates::type>((states_ids_t *)0), 0)...};
#endif
	}
	template <class T>
...
template <class T, class TEvent, class Tsm, class TDeps>
constexpr decltype(auto) get_arg(const aux::type<const TEvent &> &, const back::unexpected_event<T, TEvent> &event,
                                 Tsm &, TDeps &)
{
	return event.event_;
}
template <class T, class TEvent, class Tsm, class TDeps>
constexpr decltype(auto) get_arg(const aux::type<const TEvent &> &, const back::on_entry<T, TEvent> &event, Tsm &,
                                 TDeps &)
{
	return event.event_;
}
template <class T, class TEvent, class Tsm, class TDeps>
constexpr decltype(auto) get_arg(const aux::type<const TEvent &> &, const back::on_exit<T, TEvent> &event, Tsm &,
                                 TDeps &)
{
	return event.event_;
}
template <class T, class TEvent, class Tsm, class TDeps>
constexpr decltype(auto) get_arg(const aux::type<const TEvent &> &, const back::exception<T, TEvent> &event, Tsm &,
                                 TDeps &)
{
...
class seq_ : operator_base {
  ...
  private:
	...
	{
#if defined(__cpp_fold_expressions)
		(call<TEvent, args_t<Ts, TEvent>, typename Tsm::logger_t>::execute(aux::get_by_id<Ns>(&a), event, sm, deps,
		                                                                   subs),
		 ...);
#else
		(void)aux::swallow{0, (call<TEvent, args_t<Ts, TEvent>, typename Tsm::logger_t>::execute(aux::get_by_id<Ns>(&a),
		                                                                                         event, sm, deps, subs),
		                       0)...};
#endif
	}
...
class and_ : operator_base {
  ...
  private:
	...
	{
#if defined(__cpp_fold_expressions)
		return (call<TEvent, args_t<Ts, TEvent>, typename Tsm::logger_t>::execute(aux::get_by_id<Ns>(&g), event, sm,
		                                                                          deps, subs)
		        && ...);
#else
		auto result = true;
		(void)aux::swallow{0, (result = result
...
struct transition<state<S1>, state<S2>, front::event<E>, G, A> {
	...
	template <class TEvent, class SM, class TDeps, class TSubs>
	constexpr bool execute(const TEvent &event, SM &sm, TDeps &deps, TSubs &subs, typename SM::state_t &current_state,
	                       aux::true_type)
	{
...
struct transition<state<S1>, state<S2>, front::event<E>, always, none> {
	...
	template <class TEvent, class SM, class TDeps, class TSubs>
	constexpr bool execute(const TEvent &, SM &sm, TDeps &deps, TSubs &subs, typename SM::state_t &current_state,
	                       aux::false_type)
	{
		update_current_state(sm, deps, subs, current_state,
		                     aux::get_id<typename SM::state_t, dst_state>((typename SM::states_ids_t *)0),
